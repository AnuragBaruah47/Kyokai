  ...
  return (
    ...
    <div className="grid  w-full gap-2 justify-center p-4 grid-cols-[repeat(5,330px)]">
      {allAnime?.map((each, index) => {
        return <Link key={index} to={`${each.mal_id}`}>
          <div className="m-2"><AnimeCard imageUrl={each?.images?.webp?.large_image_url}
          title={each.title}
          type={each?.type}
          status={each.status}
          ratings={each?.rating}
          popularity={each?.popularity}/></div>
        </Link>;
      })} 
    </div>
        <div className="flex gap-5 pb-5 justify-center">
     <button onClick={prevPage} className="shadow-[5px_5px_0_#000] font-semibold px-10 py-3 rounded-md bg-white">Prev</button>
      <button onClick={nextPage} className="shadow-[5px_5px_0_#000] font-semibold px-10 py-3 rounded-md bg-white">Next</button>
      </div>
    </div>
